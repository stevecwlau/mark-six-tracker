  ...
  const renderBall = (num: number, isMatched: boolean, isExtraMatched: boolean) => {
    if (isMatched) {
      return (
        <span
          key={num}
          className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full text-white text-xs sm:text-sm font-black flex items-center justify-center shadow-md relative shrink-0 ${getBallBgColor(num)}`}
    ...
    if (isExtraMatched) {
      return (
        <span
          key={num}
          className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full text-white text-xs sm:text-sm font-black flex items-center justify-center shadow-md relative shrink-0 ring-2 ring-amber-400 ring-offset-1 ${settings.theme === 'dark' ? 'ring-offset-gray-950' : 'ring-offset-white'} ${getBallBgColor(num)}`}
    ...
    return (
      <span
        key={num}
        className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full border flex items-center justify-center text-xs sm:text-sm font-semibold select-none shrink-0 ${
          settings.theme === 'dark'
                ...
                >
                  ...
                  <div className="md:col-span-4 flex md:justify-center flex-wrap items-center gap-1.5 shrink-0">
                    {draw.numbers.map((v, idx) => (
                      <span
                        key={idx}
                        className={`w-8 h-8 rounded-full text-[10px] text-white font-bold flex items-center justify-center ${getBallBgColor(v)}`}
                      >
                        {v}
                      </span>
                    ))}
                    ...
                    <span
                      className={`w-8 h-8 rounded-full text-[10px] text-white font-black flex items-center justify-center ${getBallBgColor(draw.extraNumber ?? draw.extra)}`}
                    >
